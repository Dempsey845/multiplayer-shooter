...
var player_scene: PackedScene = preload("uid://1ugmeslkak73")
...
func _ready() -> void:
	...
	multiplayer_spawner.spawn_function = func(data):
		...
		player.name = str(data.peer_id) # needs to have the SAME node path on all of the clients
		player.set_display_name(data.display_name)
		
		# Setting the multiplayer authority to that peer
		# That means that, that peer owns this component
		# And that the server (in case of client) is not the is_multiplayer_authority of that component
		...
		return player # spawn function needs a node that hasn't been added to the scene tree
...
# call_remote means that the server cannot call that rpc
# whereas call_local allows the rpc function to be called over the network AND locally (e.g. via server peer)
